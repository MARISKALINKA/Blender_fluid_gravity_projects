...
PARAMS = {
    ...
    # emitter fully inside inlet pipe
    "inflow_location": (-3.0, 0.0, 2.4),
    "inflow_size": 0.12,
    "inflow_rotation_deg": (0.0, 90.0, 0.0),
    "inflow_velocity_xyz": (12.0, 0.0, 0.0),
    "inflow_sampling_substeps": 6,

    # remover near far end of outlet pipe
    "outflow_location": (3.15, 0.0, 0.8),
    "outflow_scale": (0.14, 0.10, 0.10),
...
def add_plane(name, location, size=1.0, rotation=(0, 0, 0)):
...
def add_cutter_cylinder(name, location, radius, length, rotation, vertices=96):
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=vertices,
        radius=radius,
        depth=length,
...
# =========================================================
# CUT OPENINGS IN TANK WALL
# =========================================================
# cutter radius is slightly smaller than outer pipe radius but
# larger than inner opening, so it cuts a proper wall opening

inlet_cutter = add_cutter_cylinder(
    "InletCutter",
    PARAMS["inlet_pipe_location"],
    PARAMS["inlet_pipe_radius"] - PARAMS["pipe_wall_thickness"] * 0.35,
    PARAMS["inlet_pipe_length"] + 0.30,
    deg_to_rad(PARAMS["inlet_pipe_rotation_deg"]),
    PARAMS["pipe_vertices"]
)
...
outlet_cutter = add_cutter_cylinder(
    "OutletCutter",
    PARAMS["outlet_pipe_location"],
    PARAMS["outlet_pipe_radius"] - PARAMS["pipe_wall_thickness"] * 0.35,
    PARAMS["outlet_pipe_length"] + 0.30,
    deg_to_rad(PARAMS["outlet_pipe_rotation_deg"]),
    PARAMS["pipe_vertices"]
)
